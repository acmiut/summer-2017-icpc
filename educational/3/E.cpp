...
int main() {
    ...
    for(int i = 1; i <= n; i++) { // assume that lookub is published after ith point
        ...
        already_published.insert(lookup); // already_published is set of all published scores base on this situation
        int tmp = lookup, num_of_found_published = 1
        for(int j = i - 1; j > 0; j--) { // check before ith score
            tmp -= scores[j];
        ...
        int tmp2 = lookup;
        for(int j = i; j < n; j++) { // check after ith score
            tmp2 += scores[j];
